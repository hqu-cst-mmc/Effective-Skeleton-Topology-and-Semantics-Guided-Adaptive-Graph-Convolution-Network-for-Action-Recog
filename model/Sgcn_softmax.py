...
# def gen_sem( n, v, t):
#
#         y = torch.arange(v).unsqueeze(-1)
#         y_onehot = torch.FloatTensor(v, v)
#
#         y_onehot.zero_()
#         y_onehot.scatter_(1, y, 1)
#
#         y_onehot = y_onehot.unsqueeze(0).unsqueeze(0)
#         y_onehot = y_onehot.repeat(n, t, 1, 1)  #N,T V,V
#         y_onehot = y_onehot.permute(0, 3, 2, 1).cuda() #N C(v) V T
#
#
#         y_onehot = embed(v, 64, norm=False, bias=True)
#         return y_onehot
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l2 = TCN_GCN_unit(128, 128, A, adaptive=adaptive, attention=attention)
        self.l3 = TCN_GCN_unit(128, 128, A, adaptive=adaptive, attention=attention)
    ...
    def forward(self, x):
        ...
        y_onehot = y_onehot.unsqueeze(0).unsqueeze(0)
        y_onehot = y_onehot.repeat(N*M, T, 1, 1)  # N,T V,V
        y_onehot = y_onehot.permute(0, 3, 2, 1).cuda()#N C(v) V T
        gen = embed(V, 64, norm=False, bias=True)  # N 64 V T 网络
        gen.cuda()
        a = gen(y_onehot)  # 输入数据
        a = a.permute(0, 1, 3, 2).contiguous().cuda() #N C T V

        x = torch.cat([x, a], dim=1) #N 128 C T
